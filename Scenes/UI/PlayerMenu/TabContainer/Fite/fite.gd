...
signal action_selected(action : PlayerAttack)
...
func _set_available_attacks(available_attacks : Array[PlayerAttack]):
...
func create_menu_select_element(attack : PlayerAttack):
...
func _update_desc(action : PlayerAttack):
	desc_label.text = action.description

func _on_action_selected(attack : PlayerAttack):
